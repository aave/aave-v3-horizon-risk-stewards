...
deploy-pk :; FOUNDRY_PROFILE=${chain} forge script $(if $(filter zksync,${chain}),--zksync) ${contract} --rpc-url ${chain} $(if ${dry},--sender ${SENDER} -vvvv, --private-key ${PRIVATE_KEY} --verify -vvvv --slow --broadcast)
